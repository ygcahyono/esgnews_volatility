...
class LSTM_Model_MS_II():
    ...
    def __init__(self, train_df, test_df, start, end, sector = 'Consumer cyclicals', features = 'm1',
                 ...
                 activation_function = 0.001, individual_asset = [True, 8589934212]
                 ):
        ...
        self.individual_asset = individual_asset
    ...
    def unique_sector(self):
        ...
        if self.individual_asset[0] is not True:
            ...
            self.list_asset = [self.individual_asset[1]]
            self.ts = info_df[info_df.Assets == self.individual_asset[1]]['Firm Name'].tolist()
            print(f'train this: {self.individual_asset[1]} or {self.ts}')
    ...
    def prepare_test_train(self):
        ...
        self.yt = np.concatenate(self.yt)
    ...
    def create_p_test_train(self):
        '''
        Prepare train and test data for time series.
        '''
        BATCH_SIZE = self.batch_size

        # Avoid shuffling for time series data to maintain sequence order
        p_train = tf.data.Dataset.from_tensor_slices((self.xtrain, self.ytrain))
        self.p_train = p_train.cache().batch(BATCH_SIZE, drop_remainder=True).repeat()

        p_test = tf.data.Dataset.from_tensor_slices((self.xtest, self.ytest))
        self.p_test = p_test.batch(BATCH_SIZE).repeat()
    
    def create_rnn_model(self, 
                         hu = 4, lags= 60, layer='LSTM', depths = 0
                         , features=3, output_size = 1, algorithm='estimation'
                         , activation = 'linear', seed = 100
                         ):
        
        # opt = tf.keras.optimizers.legacy.Adam(learning_rate=0.001)
        optmimiser = Adam(learning_rate=0.001)
        regulariser = l1(0.01)
        init_norm = glorot_normal(seed=seed)
        actfunc = LeakyReLU(self.activation_function) 
        # actfunc = 'tanh' 

        def generate_sequence(depths):
            return [2**i for i in range(depths + 1, 1, -1)]

        depth_sequences = []

        for depth in range(depths, 0, -1):
            depth_sequence = generate_sequence(depth)
            depth_sequences.append(depth_sequence)

        hu = depth_sequences[0]
        
        return_seq_length = len(hu)
        def check_sequence(return_seq_length):
            return False if return_seq_length == 0 else True

        self.model = Sequential()
        if layer == 'LSTM':
            
            return_seq_length-=1
            return_sequences = check_sequence(return_seq_length)

            self.model.add(LSTM(hu[0], return_sequences=return_sequences, activation=actfunc, 
                            kernel_initializer=init_norm, input_shape=(lags, features)
                        ))
        ...
        if algorithm == 'estimation':
            ...
            for depth in range(1,depths):
                return_seq_length-=1
                return_sequences = check_sequence(return_seq_length)
                print(hu[depth], return_sequences)
                self.model.add(LSTM(hu[depth], return_sequences = return_sequences, kernel_initializer=init_norm
                                    , activation=actfunc))
            ...
            self.model.compile(optimizer=optmimiser, loss='mse', metrics=['mse'])
    ...
    def model_LSTM(self):
        ...
        self.model = self.create_rnn_model(hu = self.hu, lags = self.past_history, depths = self.depth,
                                           layer='LSTM', features = len(self.cols) - 1, output_size=self.forward_look)
        self.create_p_test_train()

        if self.early_stop:
            self.hist = self.model.fit(self.p_train, epochs = self.epochs, steps_per_epoch = len(self.xtrain) // self.batch_size,
                    validation_data = self.p_test, validation_steps = len(self.xtest) // self.batch_size,
    ...
    def infer_values(self, xtest, ytest, ts = None):
        ...
        RMS_error_train = self.RMS_error_train

        # return pred, pred_update, RMS_error_update, RMS_error
        return pred, RMS_error, predict_days, RMS_error_train
    ...
    def vis_line_plot_results(self, y_test, y_pred, name, r, predict_days):
        ...
        plt.savefig(f'../outputs/{algorithms}-{features}/exp-sectors/{str(r+1).zfill(3)}-{algorithms}-NN{self.depth}-LB{self.past_history}-{name}.png')
        plt.close()
    ...
    def full_workflow_and_plot(self, model = None):
        ...
        for i, name  in enumerate(self.ts):
            pred, RMS_error, predict_days, RMS_error_train = self.infer_values(self.xt[i], self.yt[i], self.ts[i])
            ...
            mresult = pd.DataFrame({
                ...
                'RMS_error_valid': round(RMS_error, 10),
                'RMS_error_train': round(RMS_error_train, 10),
                'NumPred': len(pred[1:]),
                'Firm': self.ts[i]
                }, index = [i])
        ...
        mresults.to_csv(f'../results/exp-sectors/LSTM-{self.features}-{self.sector}-N{self.depth}-LB{self.past_history}-{i}-{tn}.csv')